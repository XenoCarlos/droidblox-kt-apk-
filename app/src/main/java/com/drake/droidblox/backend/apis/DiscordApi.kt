...
class DiscordApi {
    companion object {
        suspend fun fetchMPOfUrls(
            ...
            val requestTo = "https://discord.com/api/v9/applications/$droidbloxApplicationId/external-assets"
            ...
            try {
                ...
                if (mediaProxyReq.status.value != 200) {
                    Log.e(TAG, "Failed to get media proxy of urls. Got ${mediaProxyReq.status.value}\nText:\n${mediaProxyReq.bodyAsText()}")
                } else {
        ...
        suspend fun fetchUsername(
            ...
            try {
                ...
                if (usernameReq.status.value != 200) {
                    Log.e(TAG, "Failed to fetch the discord username of token. Got ${usernameReq.status.value}\nText:\n${usernameReq.bodyAsText()}")
                } else {
        ...
        suspend fun logout(
            ...
            val jsonToSend = Json.encodeToString(mapOf(
                "provider" to null,
            ...
            try {
                ...
                if (logoutReq.status.value != 204) {
                    Log.e(TAG, "Failed to logout discord token! Got ${logoutReq.status.value}\nText:\n${logoutReq.bodyAsText()}")
                }
            } catch (e: Exception) {
