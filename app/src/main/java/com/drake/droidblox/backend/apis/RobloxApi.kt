...
class RobloxApi {
    companion object {
        suspend fun fetchGameInfo(
            ...
            val requestTo = "https://games.roblox.com/v1/games?universeIds=${universeIds.joinToString(",")}"
            try {
                ...
                if (gamesInfoReq.status.value != 200) {
                    Log.e(TAG, "Failed to get roblox game info. Got ${gamesInfoReq.status.value}\nText:\n${gamesInfoReq.bodyAsText()}")
                } else {
        ...
        suspend fun fetchUserInfo(
            ...
            try {
                ...
                if (usernameReq.status.value != 200) {
                    Log.e(TAG, "Failed to fetch roblox user info. Got ${usernameReq.status.value}\nText:\n${usernameReq.bodyAsText()}")
                } else {
        ...
        suspend fun fetchThumbnails(
            ...
            try {
                ...
                if (thumbnailsReq.status.value != 200) {
                    Log.e(TAG, "Failed to get roblox thumbnail(s). Got ${thumbnailsReq.status.value}\nText:\n${thumbnailsReq.bodyAsText()}")
                } else {
                    return (thumbnailsReq.body<Map<String, List<Map<String, Any>>>>()["data"] ?: emptyList()).mapNotNull {
